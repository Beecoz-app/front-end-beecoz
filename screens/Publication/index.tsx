...
export type PublicationType = NativeStackScreenProps<BottomParamsList, 'publicationBottom'>

export const PublicationScreen = ({navigation}: PublicationType) => {
  const { setPublications } = useContext(
    PublicationContext
  ) as IPublicationContext;
  const { user } = useContext(AuthContext) as IAuthContext;
    const [titleText, setTitleText] = useState('');

  const DATA = [{ id: 1, name: "Marcenaria" }];
  ...
  return (
    <Container>
      <Content>
        ...
        <AppSpecificButton disabled={false} title="Criar pedido" onClick={handleAddPublication}/>
      </Content>
    </Container>
  );
};
